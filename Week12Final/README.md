...
### Summary
This lab involved the implementation of two clock dividers. The first was a modulo counter, which required a set of adders, D-flipflops, and comparator logic to count between 0 and 5 (modulo 6). The adders increment the value stored by 1, while the D-flipflops store the current sum. All of the D-flipflops are driven by the same clock. A synchronous reset (driven by detection of the output value 5) and an asynchronous reset (driven by a button press) were also implemented. The value of the comparator is XOR'd with the Q of a D-flipflop, the value of which is fed into the D of the same; this Q is used as the new divided clock output. This allows for the output to toggle from high for 6 clock cycles to low for 6 clock cycles.

The second clock divider was a ripple counter, created using a series of t-flipflops. The output (Q) of the first T-flipflop drives the clock of the next T-flipflop. Chaining N T-flipflops will divide the input clock by 2^N. The T-flipflops also include
 a reset input which is piped through each to the internal D-flipflops to reset each individual memory bit.

### 1 - Why does the Modulo Counter actually divide clocks by 2 * Count?
The modulo counter divides the clock by 2 * count because the output for the whole modulo divider comes from the final D-flipflop, which toggles on the modulo because of the XOR configuration. This means that, every 6 clock cycles, the final output transitions from its current state to the opposite state. One Hz encompasses a transition from high to low and back to high. So two counts are required per one Hz.
